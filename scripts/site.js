...
Y.Template.noYUI = {
	...
	/**
	 * The "Transparent Header" tweak option absolutely positions the header (logo + nav) atop the first index section. 
	 * If the first index section has a background image, we need to add some extra top-padding to the section's content 
	 * to vertically center it between the bottom of the header and the top of the next index section. 
	 * 
	...
	vCenterTopSectionContent: function() {
		...
		if ($topSection) {
			...
			if (headerPosition == 'absolute' && isTopSectionWithMainImage && !isTopSectionWithGallery) {
				...
				// by default, the nav wraps around the left/right side of the logo image. if there's not enough room,
				// it drops below the logo image. in order to figure out which state it's in...
				
				if($nav){
					// temporarily force the nav into one line and add the width of the logo image and spacing around it 
					// and store the value. this is its width as if we never let it drop below the logo image.
					...
					// compare the header width to the tempNavWidth to see if it would be too big to fit inside the header.
					// if so, we can safely assume that it's dropped below the logo image, so we need to add the nav's height 
